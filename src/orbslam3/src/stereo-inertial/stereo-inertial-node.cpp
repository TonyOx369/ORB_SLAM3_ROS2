    ...
    SLAM_(SLAM)
{
    ...
    // subImgLeft_ = this->create_subscription<ImageMsg>("/zed/zed_node/left/image_rect_color", 100, std::bind(&StereoInertialNode::GrabImageLeft, this, _1));
    // subImgRight_ = this->create_subscription<ImageMsg>("/zed/zed_node/right/image_rect_color", 100, std::bind(&StereoInertialNode::GrabImageRight, this, _1));
...
cv::Mat StereoInertialNode::GetImage(const ImageMsg::SharedPtr msg)
{
    ...
    try
    {
        // Request the encoding ZED provides (bgra8)
        cv_ptr = cv_bridge::toCvShare(msg, sensor_msgs::image_encodings::BGRA8);
    }
    catch (cv_bridge::Exception &e)
    {
    ...
    {
        // Convert BGRA → BGR
        cv::cvtColor(cv_ptr->image, img, cv::COLOR_BGRA2BGR);
    }
    else if (cv_ptr->image.channels() == 1)
    {
        img = cv_ptr->image.clone();  // already mono
    }
    else
    {
        std::cerr << "Unexpected image format: " << cv_ptr->image.type() << std::endl;
